...
-- Table: AEAPRREC
CREATE TABLE IF NOT EXISTS AEAPRREC (ID_AEAPRREC INTEGER PRIMARY KEY ON CONFLICT REPLACE NOT NULL, ID_AEAPRODU INTEGER NOT NULL, ID_CFAAREAS INTEGER, ID_CFACIDAD INTEGER, ID_CFACLIFO_VENDEDOR INTEGER, ID_CFACLIFO INTEGER, ID_SMAEMPRE INTEGER, POSICAO INTEGER);
...
CREATE INDEX INDEX_CFAFOTOS_ID_CFAFOTOS_ASC ON CFAFOTOS (ID_CFAFOTOS ASC);

-- Index: INDEX_CFAFOTOS_ID_AEAPRODU_DESC
CREATE INDEX INDEX_CFAFOTOS_ID_AEAPRODU_DESC ON CFAFOTOS (ID_AEAPRODU DESC);
...
CREATE INDEX INDEX_AEAPRRE_ID_AEAPRREC_ASC ON AEAPRREC (ID_AEAPRREC ASC);
...
WHERE (ID_AEAESTOQ = NEW.ID_AEAESTOQ)) || ', com a quantidade ' || NEW.QUANTIDADE || ', Pedido/Orcamento de N. ' || NEW.ID_AEAORCAM)$ END~^
^
...
^^BEGIN^^
	...
	SELECT CASE WHEN^
			/* Checa se existe o cadastro no CPF/CNPJ */
