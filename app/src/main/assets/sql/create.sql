...
-- Table: AEAITORC
CREATE TABLE IF NOT EXISTS "AEAITORC" (
ID_AEAITORC  INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,
ID_AEAORCAM  INTEGER NOT NULL,
ID_AEAESTOQ  INTEGER,
ID_AEAPRODU  INTEGER,
ID_AEAPLPGT  INTEGER NOT NULL,
ID_AEAUNVEN  INTEGER NOT NULL,
ID_CFACLIFO_VENDEDOR  INTEGER NOT NULL,
GUID  CHAR(16) NOT NULL,
DT_CAD  DATETIME DEFAULT ((DATETIME('NOW', 'localtime'))),
DT_ALT  DATETIME,
...
VL_BRUTO  REAL NOT NULL DEFAULT '0',
VL_DESCONTO  REAL NOT NULL DEFAULT '0',
VL_TABELA  REAL,
VL_TABELA_UN  REAL,
VL_TABELA_FATURADO  REAL,
VL_TABELA_UN_FATURADO  REAL,
FC_CUSTO_UN  REAL,
FC_BRUTO_UN  REAL,
FC_DESCONTO_UN  REAL,
FC_LIQUIDO  REAL,
FC_LIQUIDO_UN  REAL,
FC_LIQUIDO_FATURADO  REAL,
PROMOCAO  CHAR(1),
TIPO_PRODUTO  CHAR(1) NOT NULL,
COMPLEMENTO  TEXT,
SEQ_DESCONTO  VARCHAR(50),
PESO_LIQUIDO  DOUBLE DEFAULT (0),
PESO_BRUTO  DOUBLE DEFAULT (0),
STATUS_RETORNO  CHAR);
...
-- Table: CFACCRED
CREATE TABLE IF NOT EXISTS CFACCRED (ID_CFACCRED INTEGER NOT NULL UNIQUE, DT_ALT DATETIME, CODIGO INTEGER NOT NULL, DESCRICAO VARCHAR (40) NOT NULL);

-- Table: RPAPARCE
CREATE TABLE IF NOT EXISTS RPAPARCE (ID_RPAPARCE INTEGER NOT NULL UNIQUE, ID_SMAEMPRE INTEGER NOT NULL, ID_RPAFATUR INTEGER, ID_CFACLIFO INTEGER NOT NULL, ID_CFATPDOC INTEGER NOT NULL, ID_CFATPCOB INTEGER NOT NULL, ID_CFAPORTA INTEGER NOT NULL, DT_ALT DATETIME, TIPO CHAR (1) NOT NULL, DT_EMISSAO DATE NOT NULL, DT_VENCIMENTO DATE NOT NULL, DT_BAIXA DATE, PARCELA INTEGER NOT NULL DEFAULT '0', VL_PARCELA REAL NOT NULL DEFAULT (0), FC_VL_TOTAL_PAGO REAL NOT NULL DEFAULT '0', FC_VL_RESTANTE REAL NOT NULL DEFAULT '0', VL_JUROS_DIARIO REAL NOT NULL DEFAULT '0', PERC_DESCONTO REAL NOT NULL DEFAULT '0', SEQUENCIAL VARCHAR (13) NOT NULL, NUMERO VARCHAR (20), OBS TEXT);
...
-- Table: USUARIO_USUA
CREATE TABLE IF NOT EXISTS USUARIO_USUA (ID_USUA INTEGER NOT NULL, ID_SMAEMPRE INTEGER, DATA_ALTERACAO_USUA DATETIME, CHAVE_USUA VARCHAR, NOME_USUA VARCHAR, LOGIN_USUA VARCHAR NOT NULL, SENHA_USUA VARCHAR NOT NULL, EMAIL_USUA VARCHAR NOT NULL, EMPRESA_USUA VARCHAR, VENDE_ATACADO_USUA CHAR (1) DEFAULT '1', VENDE_VAREJO_USUA CHAR (1) DEFAULT '0', ATIVO_USUA CHAR (1), IP_SERVIDOR_USUA VARCHAR, USUARIO_SERVIDOR_USUA VARCHAR, SENHA_SERVIDOR_USUA VARCHAR, PASTA_SERVIDOR_USUA VARCHAR DEFAULT '/', DT_ULTIMO_RECEBIMENTO DATETIME, DT_ULTIMO_ENVIO DATETIME DEFAULT NULL, VALOR_CREDITO_ATACADO REAL DEFAULT 0, VALOR_CREDITO_VAREJO REAL DEFAULT 0, PERCENTUAL_CREDITO_ATACADO REAL DEFAULT 0, PERCENTUAL_CREDITO_VAREJO REAL DEFAULT 0);
...
-- Table: AEAMARCA
CREATE TABLE IF NOT EXISTS AEAMARCA (ID_AEAMARCA INTEGER NOT NULL UNIQUE, DT_ALT DATETIME, DESCRICAO VARCHAR (40) NOT NULL);
...
-- Index: INDEXx_AEAPLOJA_ID_AEAPLOJA
CREATE INDEX IF NOT EXISTS INDEXx_AEAPLOJA_ID_AEAPLOJA ON AEAPLOJA (ID_AEAPLOJA);
...
CREATE INDEX IF NOT EXISTS INDEX_AEAORCAM_NUMERO ON AEAORCAM (NUMERO);
...
-- Trigger: UPDATE_AEAITORC_ANT
CREATE TRIGGER IF NOT EXISTS UPDATE_AEAITORC_ANT BEFORE UPDATE ON AEAITORC^^
^^BEGIN^^^^
SELECT CASE WHEN ^
		( ((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		(( SELECT AEAPLOJA.PRECO_MINIMO_ATAC FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) AND (AEAPLOJA.PRECO_MINIMO_ATAC > 0)) IS NOT NULL)^
		AND ^
		( NEW.FC_LIQUIDO_UN < ( SELECT AEAPLOJA.PRECO_MINIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))  ) ^
		^
	^THEN^ ^
		RAISE (ABORT, 'Preço de Venda Menor que o mínimo permitido para as vendas no ATACADO.' ) ^
END;^
^
^
^
SELECT CASE WHEN ^
		...
		AND^
		(( SELECT AEAPLOJA.PRECO_MINIMO_VARE FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) AND (AEAPLOJA.PRECO_MINIMO_VARE > 0)) IS NOT NULL)^
		AND ^
		( NEW.FC_LIQUIDO_UN < ( SELECT AEAPLOJA.PRECO_MINIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) )  ) ^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Menor que o mínimo permitido para as vendas no VAREJO.' ) ^
END;^
^
^
^
SELECT CASE WHEN ^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		( ( SELECT AEAPLOJA.PRECO_MAXIMO_ATAC FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPRODU = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) AND (AEAPLOJA.PRECO_MAXIMO_ATAC > 0) ) IS NOT NULL )^
		AND ^
		( NEW.FC_LIQUIDO_UN > (SELECT AEAPLOJA.PRECO_MAXIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) ) )^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Maior que o máximo permitido no ATACADO.' ) ^
END;^
^
^
^
SELECT CASE WHEN ^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		AND^
		((SELECT AEAPLOJA.PRECO_MAXIMO_VARE FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPRODU = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) AND (AEAPLOJA.PRECO_MAXIMO_VARE > 0)) IS NOT NULL )^
		AND ^
		( NEW.FC_LIQUIDO_UN > ( SELECT AEAPLOJA.PRECO_MAXIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) ) ) ^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Maior que o máximo permitido no VAREJO.' ) ^
END;^
^
^
^
...
SELECT CASE WHEN^
		...
		AND^
		((SELECT SMAEMPRE.MULTIPLOS_PLANOS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ))) IS NOT NULL)^
		AND^
		((SELECT SMAEMPRE.MULTIPLOS_PLANOS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ))) = '0')^
		AND^
		(NEW.ID_AEAPLPGT <> (SELECT AEAITORC.ID_AEAPLPGT FROM AEAITORC WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM ORDER BY AEAITORC.SEQUENCIA DESC LIMIT 1)))^
	^THEN^^
		RAISE(ABORT, 'Não é permitido ter multiplos planos de pagamentos(Prazos) em um mesmo orçamento/pedido, retorne e escolha o mesmo plano de pagamento que o produto anterior.')^
	END;^
^
END~^
^
...
^BEGIN^ ^
	SELECT CASE WHEN ^
					((NEW.STATUS = 'P') ^
					AND ^
					((SELECT COUNT() FROM AEAITORC WHERE AEAITORC_OLD.ID_AEAORCAM = NEW.ID_AEAORCAM) <= 0))^
	...
	SELECT CASE WHEN ^
					((NEW.STATUS = 'P') ^
					AND ^
					((SELECT AVG(AEAPLPGT.DIAS_MEDIOS) FROM AEAITORC LEFT OUTER JOIN AEAPLPGT ON (AEAITORC_OLD.ID_AEAPLPGT = AEAPLPGT.ID_AEAPLPGT) WHERE AEAITORC_OLD.ID_AEAORCAM = NEW.ID_AEAORCAM) > 0) ^
					AND ^
					((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0') ^
					AND ^
	...
	SELECT CASE WHEN ^
					...
					AND ^
					(((SELECT SUM(RPAPARCE.FC_VL_RESTANTE) FROM RPAPARCE WHERE (RPAPARCE.ID_CFACLIFO = NEW.ID_CFACLIFO) AND (RPAPARCE.TIPO = 0) AND (RPAPARCE.DT_BAIXA IS NULL)) + NEW.FC_VL_TOTAL) > (SELECT CFAPARAM.LIMITE FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = NEW.ID_CFACLIFO))) ^
				^THEN^ RAISE (ABORT, 'O valor do orçamento ultrapassa o limite de compra da pessoa.') ^
				END; ^
				^
	SELECT CASE WHEN ^
					((NEW.STATUS = 'P') ^
	...
	SELECT CASE WHEN ^
				...
				^THEN^ RAISE (ABORT, 'Venda não permitida por ultrapassar o limite de dias de atrazo.') ^
				END; ^
END~^
^
	...
	VALOR_CREDITO_ATACADO = ^
					(CASE WHEN(^
							...
							WHEN (^
								...
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-14 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-14 23:59:59')) || "'")), 0))^
								)^
								^
							WHEN (^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
								...
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-15 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-31 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-31 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
							^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-0NEW.ID_AEAORCAM-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-0NEW.ID_AEAORCAM-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
							^
					END^
					)^
	...
	VALOR_CREDITO_VAREJO = ^
					(CASE WHEN(^
							...
							WHEN (^
								...
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-14 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-14 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
								)^
								^
							WHEN (^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
							...
							WHEN(^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-06-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-06-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
								)^
							^
					END^
					)^
	...
	PERCENTUAL_CREDITO_ATACADO =^
					(CASE WHEN(^
							...
							WHEN (^
								...
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-01 00:00:00')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-' || STRFTIME('%m', 'NOW', 'LOCALTIME') || '-14 23:59:59')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
								^
							WHEN (^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
							...
							WHEN(^
								...
								^
								^THEN^ (^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL)) / (SUM(VL_TABELA)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-06-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT ID_SMAEMPRE FROM AEAORCAM WHERE ID_AEAORCAM = NEW.ID_AEAORCAM)), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-01-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-06-30')) || "'") AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
								^
							WHEN(^
								((SELECT AEAORCAM.ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
...
CREATE TRIGGER IF NOT EXISTS "INSERT_AEAITORC" AFTER INSERT ON "AEAITORC"^
^BEGIN^ ^
UPDATE AEAITORC SET DT_ALT = DATETIME('now' , 'localtime') WHERE (ROWID = NEW.ROWID)$^
^
^
^
UPDATE AEAORCAM SET ^
...
UPDATE AEAITORC SET ^
		...
		PESO_BRUTO = ROUND(IFNULL(((SELECT AEAPRODU.PESO_LIQUIDO FROM AEAPRODU, AEAPLOJA, AEAESTOQ WHERE AEAPRODU.ID_AEAPRODU = AEAPLOJA.ID_AEAPRODU AND AEAPLOJA.ID_AEAPLOJA = AEAESTOQ.ID_AEAPLOJA AND AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) * NEW.QUANTIDADE), 0), 3)^
...
^
INSERT INTO LOG (ID_TABELA, TABELA, OPERACAO, USUARIO, VALORES) ^
VALUES (NEW.ID_AEAORCAM, 'AEAITORC', 'I', (SELECT NOME_USUA FROM USUARIO_USUA WHERE ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 'Inseriu o produto ' || ^
(SELECT AEAPRODU.CODIGO_ESTRUTURAL || ' - ' || AEAPRODU.DESCRICAO FROM AEAESTOQ ^
...
WHERE (ID_AEAESTOQ = NEW.ID_AEAESTOQ)) || ', com a quantidade ' || NEW.QUANTIDADE)$ END~^
^
...
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
								)^
^
							WHEN (^
^
...
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
^
								)^
^
								^
...
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
^
								)^
^
							WHEN (^
...
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
^
								)^
^
								^
...
^
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA)) - (SUM(FC_VL_TOTAL))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'")), 0)) + ^
^
								(IFNULL((SELECT ROUND(((SUM(VL_TABELA_FATURADO)) - (SUM(FC_VL_TOTAL_FATURADO))), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND (AEAORCAM.DT_CAD >= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-07-01')) || "'") AND (AEAORCAM.DT_CAD <= "'" || (SELECT DATETIME(STRFTIME('%Y', 'NOW', 'LOCALTIME') || '-12-31')) || "'") AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
^
								)^
...
^
								(SELECT (IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL)) / (SUM(VL_TABELA)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0))) + ^
^
								(SELECT (IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0)))^
^
								)^
^
								^
...
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL)) / (SUM(VL_TABELA)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0)) + ^
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '0')), 0))^
^
								)^
^
								^
...
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL)) / (SUM(VL_TABELA)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%W', AEAORCAM.DT_CAD)) = (STRFTIME('%W', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
^
								)^
^
								^
...
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL)) / (SUM(VL_TABELA)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE ((AEAORCAM.STATUS = 'P') OR (AEAORCAM.STATUS = 'N') OR (AEAORCAM.STATUS = 'RL')) AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0)) + ^
^
								(IFNULL((SELECT ROUND((((((SUM(FC_VL_TOTAL_FATURADO)) / (SUM(VL_TABELA_FATURADO)))*100)-100)*-1), (IFNULL((SELECT QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = NEW.ID_SMAEMPRE), 3))) FROM AEAORCAM WHERE (AEAORCAM.STATUS = 'F') AND ((STRFTIME('%m', AEAORCAM.DT_CAD)) = (STRFTIME('%m', 'now', 'localtime'))) AND (AEAORCAM.ATAC_VAREJO = '1')), 0))^
^
								)^
^
								^
...
UPDATE AEAORCAM SET ^
		...
		VL_MERC_BRUTO = IFNULL((SELECT SUM ( AEAITORC.VL_BRUTO ) FROM AEAITORC WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM), 0) , ^
		VL_MERC_DESCONTO = IFNULL((SELECT SUM ( AEAITORC.VL_DESCONTO ) FROM AEAITORC WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM), 0), ^
		VL_TABELA = IFNULL((SELECT SUM (AEAITORC.VL_TABELA) FROM AEAITORC WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM), 0) ^
WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM$^
^
^
^
...
CREATE TRIGGER IF NOT EXISTS INSERT_AEAITORC_ANT BEFORE INSERT ON AEAITORC^
^BEGIN^^
SELECT CASE WHEN^
		((NEW.QUANTIDADE <= 0))^
	^THEN^^
		RAISE(ABORT, 'A quantidade do produto esta zerada ou menor que zero.')^
END;^
^
^
^
SELECT CASE WHEN ^
		( ((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		(( SELECT AEAPLOJA.PRECO_MINIMO_ATAC FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ))) IS NOT NULL)^
		AND ^
		((SELECT AEAPLOJA.PRECO_MINIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND ^
		( NEW.FC_LIQUIDO_UN < ( SELECT AEAPLOJA.PRECO_MINIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))  ) ^
		^
	^THEN^ ^
...
SELECT CASE WHEN ^
		( ((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		AND^
		(( SELECT AEAPLOJA.PRECO_MINIMO_VARE FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ))) IS NOT NULL)^
		AND ^
		((SELECT AEAPLOJA.PRECO_MINIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND ^
		( NEW.FC_LIQUIDO_UN < ( SELECT AEAPLOJA.PRECO_MINIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) )  ) ^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Menor que o mínimo permitido para as vendas no VAREJO.' ) ^
...
SELECT CASE WHEN ^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		( ( SELECT AEAPLOJA.PRECO_MAXIMO_ATAC FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) ) IS NOT NULL )^
		AND^
		((SELECT AEAPLOJA.PRECO_MAXIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND ^
		( NEW.FC_LIQUIDO_UN > (SELECT AEAPLOJA.PRECO_MAXIMO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) ) )^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Maior que o máximo permitido no ATACADO.' ) ^
...
SELECT CASE WHEN ^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		AND^
		((SELECT AEAPLOJA.PRECO_MAXIMO_VARE FROM AEAPLOJA WHERE (AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ))) IS NOT NULL )^
		AND^
		((SELECT AEAPLOJA.PRECO_MAXIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND ^
		( NEW.FC_LIQUIDO_UN > ( SELECT AEAPLOJA.PRECO_MAXIMO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ) ) ) ) ^
	^THEN^ ^
		RAISE ( ABORT , 'Preço de Venda Maior que o máximo permitido no VAREJO.' ) ^
...
WHEN^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		((SELECT PROMOCAO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND^
		((SELECT AVG(AEAPLPGT.DIAS_MEDIOS) FROM AEAITORC ^
		LEFT OUTER JOIN AEAPLPGT^
		ON(AEAITORC.ID_AEAPLPGT = AEAPLPGT.ID_AEAPLPGT)^
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) > 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT PROMOCAO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_ATAC_PRAZO FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT PROMOCAO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		^
WHEN^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '0')^
		AND^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) <= 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT PROMOCAO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_ATAC_VISTA FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT PROMOCAO_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		^
WHEN^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) > 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT VENDA_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_ATAC_PRAZO FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT VENDA_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		^
WHEN^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) <= 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT VENDA_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_ATAC_VISTA FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT VENDA_ATAC FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
END;^
^
...
WHEN^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		AND^
		((SELECT PROMOCAO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) > 0)^
		AND^
		((SELECT AVG(AEAPLPGT.DIAS_MEDIOS) FROM AEAITORC ^
		LEFT OUTER JOIN AEAPLPGT^
		ON(AEAITORC.ID_AEAPLPGT = AEAPLPGT.ID_AEAPLPGT)^
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) > 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT PROMOCAO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_VARE_PRAZO FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT PROMOCAO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		^
WHEN^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) <= 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT PROMOCAO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_VARE_VISTA FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT PROMOCAO_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
WHEN^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) > 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT VENDA_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_VARE_PRAZO FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT VENDA_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
WHEN^
		(((SELECT ATAC_VAREJO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM) = '1')^
		...
		WHERE AEAITORC.ID_AEAORCAM = NEW.ID_AEAORCAM) <= 0 ))^
	^THEN^^
		NEW.FC_LIQUIDO = ROUND(( NEW. QUANTIDADE * (((SELECT VENDA_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)) * ((SELECT DESC_VARE_VISTA FROM CFAPARAM WHERE CFAPARAM.ID_CFACLIFO = (SELECT AEAORCAM.ID_CFACLIFO FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM))/100)) + (SELECT VENDA_VARE FROM AEAPLOJA WHERE AEAPLOJA.ID_AEAPLOJA = (SELECT AEAESTOQ.ID_AEAPLOJA FROM AEAESTOQ WHERE AEAESTOQ.ID_AEAESTOQ = NEW.ID_AEAESTOQ)))), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
		AND^
		NEW.FC_LIQUIDO_UN = ROUND((NEW.FC_LIQUIDO / NEW.QUANTIDADE), (IFNULL((SELECT SMAEMPRE.QTD_CASAS_DECIMAIS FROM SMAEMPRE WHERE SMAEMPRE.ID_SMAEMPRE = (SELECT AEAORCAM.ID_SMAEMPRE FROM AEAORCAM WHERE AEAORCAM.ID_AEAORCAM = NEW.ID_AEAORCAM)), 3)))^
END; END~
